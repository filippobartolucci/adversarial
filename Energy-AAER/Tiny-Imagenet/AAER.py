...
from tiny_architecture import PreActResNet, ti_preactresnet
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if args.delta_init == 'zero':
                delta = torch.zeros(b, 3, 64, 64).cuda()
            elif args.delta_init == 'random':
                delta_ran = torch.zeros(b, 3, 64, 64).cuda()
                if args.clamp:
    ...
    logger.info('%.4f \t \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f', test_loss, test_acc, fgsm_loss, fgsm_acc, pgd20_loss, pgd20_acc, pgd50_loss, pgd50_acc)
