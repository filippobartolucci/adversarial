...
from tiny_architecture import ti_preactresnet
...
def get_args():
    ...
    parser.add_argument('--data-dir', default='/home/fbartolucci/ADVERSARIAL/2023_NeurIPS_AAER/Tiny-imagenet/data', type=str)
...
def main():
    ...
    model = ti_preactresnet('preact-resnet18').to('cuda')
    ...
    for epoch in range(args.epochs):
        ...
        if math.isnan(train_loss / train_n):
            logger.info('Loss is nan. Exiting...')
            break
    ...
    # Evaluation
    model_test = ti_preactresnet('preact-resnet18').cuda()
    ...
    pgd20_loss, pgd50_acc = evaluate_pgd(test_loader, model_test, 20, 10, epsilon)
    pgd50_loss, pgd20_acc = evaluate_pgd(test_loader, model_test, 50, 10, epsilon)

    logger.info('Test Loss \t Test Acc \t FGSM Loss \t FGSM Acc \t PGD20 Acc\t PGD50 Acc')
    logger.info('%.4f \t \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f', test_loss, test_acc, fgsm_loss, fgsm_acc, pgd20_acc, pgd50_acc)
