    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if abnormal_count != 0 and normal_count != 0:
                loss = loss +  (args.lamda1 * abnormal_count / y.size(0)) * (args.lamda2 * abnormal_ce + args.lamda3 * max(abnormal_variation - normal_variation.item(), 0)) # '* min((epoch/20), 1)' warm-up for long training schedule
